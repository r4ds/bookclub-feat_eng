...
> "If a model is only 50% accurate should be used to make inferences or predictions?"
...
```{r echo=FALSE}
segmentationData%>%names;segmentationData%>%dim
```
...
Cros validation on the training set. 
```{r folds}
set.seed(2222)
...
roc_curves <- roc %>%
  ggplot(aes(x = 1 - specificity, y = sensitivity,group=wflow_id,color=wflow_id )) +
  geom_line(size = 0.5) +
